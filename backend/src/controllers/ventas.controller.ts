...
export const ventasController = {
    ...
    try {
      ...
      for (const row of salesRes.rows) {
        const detailsRes = await pool.query(
          `SELECT vd.*, p.producto_nombre, p.producto_codigo
  ...
  create: async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      const { empleado_id, productos } = req.body as ICreateVentaRequest;
      ...
      for (const p of productos) {
        const prodRes = await client.query(
          'SELECT * FROM producto WHERE producto_id = $1 FOR UPDATE',
          [p.producto_id]
        );
      ...
      const ventaRes = await client.query(
        `INSERT INTO venta (sucursal_id, empleado_id, usuario_id, venta_total, venta_estado, venta_observacion) 
         VALUES ($1, $2, $3, $4, 'completada', 'Compra POS Autoservicio') 
         RETURNING venta_id`,
        [sucursalId, empleado_id, usuarioId, totalNeto]
      );
      ...
      for (const d of calculatedDetails) {
        ...
        await client.query(
          `INSERT INTO venta_detalle (venta_id, producto_id, venta_detalle_cantidad, venta_detalle_precio_unitario, venta_detalle_subtotal) 
           VALUES ($1, $2, $3, $4, $5)`,
          [ventaId, d.producto_id, d.cantidad, d.precio_unitario, itemNeto]
        );
      ...
      const solRes = await client.query(
        `INSERT INTO solicitud_entrega (empleado_id, sucursal_id, solicitud_entrega_codigo, solicitud_entrega_estado, solicitud_entrega_observacion) 
         ...
         RETURNING solicitud_entrega_id`,
        [empleado_id, sucursalId, codigoRetiro]
      );
      ...
      for (const d of calculatedDetails) {
        await client.query(
          `INSERT INTO solicitud_entrega_detalle (solicitud_entrega_id, producto_id, solicitud_entrega_detalle_cantidad, solicitud_entrega_detalle_precio_unitario) 
           VALUES ($1, $2, $3, $4)`,
          [solId, d.producto_id, d.cantidad, d.precio_unitario]
        );
      }
      ...
      throw new AppError('Error al realizar compra en el servidor', 500);
    } finally {
      client.release();
    }
  }
