...
export const usuariosController = {
  ...
  getUserPermissions: async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      const id = parseInt(req.params.id);
      ...
      const customPermsRes = await pool.query(`
        SELECT permiso_id, tipo
        FROM usuario_permiso
        WHERE usuario_id = $1
      `, [id]);

      const customConcedidos = new Set<number>();
      const customDenegados = new Set<number>();
      for (const row of customPermsRes.rows) {
        if (row.tipo === 'conceder') customConcedidos.add(row.permiso_id);
        if (row.tipo === 'denegar') customDenegados.add(row.permiso_id);
      }
      ...
      throw new AppError('Error al consultar permisos del operador', 500);
    }
  },

  // Guardar personalizaciones de permisos para un usuario
  saveUserPermissions: async (req: AuthRequest, res: Response): Promise<void> => {
    const client = await pool.connect();
    try {
      const id = parseInt(req.params.id);
      ...
      if (!Array.isArray(cambios)) {
        throw new AppError('Formato de datos inválido para permisos', 400);
      }
      ...
      for (const item of cambios) {
        ...
        if (item.activo && !heredaPorRol) {
          ...
          await client.query(
            `INSERT INTO usuario_permiso (usuario_id, permiso_id, tipo) VALUES ($1, $2, 'conceder')`,
            [id, item.permiso_id]
          );
        } else if (!item.activo && heredaPorRol) {
          // Lo tenía en el rol, pero se lo denegamos específicamente
          await client.query(
            `INSERT INTO usuario_permiso (usuario_id, permiso_id, tipo) VALUES ($1, $2, 'denegar')`,
            [id, item.permiso_id]
          );
        }
      }
      ...
      await client.query('ROLLBACK');
      if (error instanceof AppError) throw error;
      throw new AppError('Error al guardar permisos personalizados', 500);
    } finally {
      client.release();
    }
  }
...
function rolePermsIdsContains(set: Set<number>, id: number): boolean {
  return set.has(id);
}
