...
exports.entregasController = {
    ...
    getAll: async (req, res) => {
      ...
      `;
           ...
           WHERE sd.solicitud_entrega_id = $1`, [row.solicitud_entrega_id]);
                ...
                const devRes = await db_1.default.query('SELECT devolucion_estado FROM devolucion WHERE solicitud_entrega_id = $1 ORDER BY devolucion_id DESC LIMIT 1', [row.solicitud_entrega_id]);
                ...
                const devSumRes = await db_1.default.query(`SELECT dd.producto_id, COALESCE(SUM(dd.cantidad_devuelta), 0) as total_devuelto
           ...
           GROUP BY dd.producto_id`, [row.solicitud_entrega_id]);
                const devSumMap = {};
            ...
            throw new error_middleware_1.AppError('Error al obtener entregas', 500);
        }
    },
    ...
    getById: async (req, res) => {
      ...
      `;
         ...
         WHERE sd.solicitud_entrega_id = $1`, [row.solicitud_entrega_id]);
            ...
            const devRes = await db_1.default.query('SELECT devolucion_estado, devolucion_motivo, devolucion_observacion_tthh FROM devolucion WHERE solicitud_entrega_id = $1 ORDER BY devolucion_id DESC LIMIT 1', [row.solicitud_entrega_id]);
         ...
         GROUP BY dd.producto_id`, [row.solicitud_entrega_id]);
            ...
            if (error instanceof error_middleware_1.AppError)
                throw error;
            throw new error_middleware_1.AppError('Error al obtener los detalles de la entrega', 500);
        }
    },
    // Obtener pendientes
    getPendientes: async (_req, res) => {
        try {
            const query = `
        SELECT s.*, e.empleado_nombre, e.empleado_apellido, e.empleado_cedula, d.departamento_nombre, e.empleado_foto
        FROM solicitud_entrega s
      ...
      `;
           ...
           GROUP BY dd.producto_id`, [row.solicitud_entrega_id]);
            ...
            throw new error_middleware_1.AppError('Error al obtener entregas pendientes', 500);
        }
    },
    ...
    confirmar: async (req, res) => {
        try {
            ...
            if (!solicitud_id || !metodo_verificacion) {
                throw new error_middleware_1.AppError('Datos de confirmación incompletos', 400);
            }
            const solRes = await db_1.default.query('SELECT * FROM solicitud_entrega WHERE solicitud_entrega_id = $1', [solicitud_id]);
            const solicitud = solRes.rows[0];
            if (!solicitud) {
                throw new error_middleware_1.AppError('Solicitud de entrega no encontrada', 404);
            }
            if (solicitud.solicitud_entrega_estado !== 'pendiente') {
                throw new error_middleware_1.AppError('La solicitud ya fue procesada anteriormente', 400);
            }
            ...
            await db_1.default.query(`UPDATE solicitud_entrega 
         ...
         WHERE solicitud_entrega_id = $4`, [observaciones || 'Entrega completada físicamente', usuarioEntregaId, foto, solicitud_id]);
            // Si existía una solicitud de devolución/cancelación pendiente para esta entrega, se rechaza automáticamente.
            await db_1.default.query(`UPDATE devolucion 
         ...
         WHERE solicitud_entrega_id = $1 AND devolucion_estado = 'pendiente'`, [solicitud_id]);
            if (req.io) {
            ...
            if (error instanceof error_middleware_1.AppError)
                throw error;
            throw new error_middleware_1.AppError('Error al confirmar la entrega en bodega', 500);
        }
    },
    ...
    cancelar: async (req, res) => {
        ...
        try {
            ...
            const solicitud = solRes.rows[0];
            if (!solicitud) {
                throw new error_middleware_1.AppError('Solicitud no encontrada', 404);
            }
            // 2. Validar que exista una devolución aprobada por Talento Humano para esta entrega
            const devRes = await client.query("SELECT * FROM devolucion WHERE solicitud_entrega_id = $1 AND devolucion_estado = 'aprobado'", [id]);
            if (devRes.rows.length === 0) {
                throw new error_middleware_1.AppError('No se puede cancelar esta entrega. Primero debe esperar que Talento Humano apruebe la solicitud de devolución.', 400);
            }
            const devolucionId = devRes.rows[0].devolucion_id;
            // 3. Restaurar el stock físico únicamente de los productos devueltos
            const devDetailsRes = await client.query('SELECT * FROM devolucion_detalle WHERE devolucion_id = $1', [devolucionId]);
            const stockUpdates = [];
            ...
            const originalDetailsCount = await client.query('SELECT COALESCE(SUM(solicitud_entrega_detalle_cantidad), 0) as total FROM solicitud_entrega_detalle WHERE solicitud_entrega_id = $1', [id]);
            const allReturnedDetailsCount = await client.query(`SELECT COALESCE(SUM(dd.cantidad_devuelta), 0) as total 
         FROM devolucion_detalle dd
         JOIN devolucion d ON dd.devolucion_id = d.devolucion_id
         WHERE d.solicitud_entrega_id = $1 AND (d.devolucion_estado = 'ejecutado' OR d.devolucion_id = $2)`, [id, devolucionId]);
            ...
            await client.query(`UPDATE solicitud_entrega 
         SET solicitud_entrega_estado = $1, 
             solicitud_entrega_observacion = $2 
         WHERE solicitud_entrega_id = $3`, [nuevoEstadoEntrega, obsEntrega, id]);
            // 5. Buscar la venta original para actualizar o anular
            const ventaQuery = await client.query(`SELECT venta_id FROM venta 
         WHERE empleado_id = $1 
           AND sucursal_id = $2 
           AND ABS(EXTRACT(EPOCH FROM (venta_fecha - $3))) < 5
           AND venta_estado = 'completada'
         LIMIT 1`, [solicitud.empleado_id, solicitud.sucursal_id, solicitud.solicitud_entrega_fecha_solicitud]);
            if (ventaQuery.rows.length > 0) {
                ...
                if (isTotalDevolucion) {
                    ...
                    await client.query(`UPDATE venta 
             ...
             WHERE venta_id = $1`, [ventaId]);
                    ...
                    for (const d of devDetailsRes.rows) {
                        const vdRes = await client.query(`SELECT * FROM venta_detalle WHERE venta_id = $1 AND producto_id = $2`, [ventaId, d.producto_id]);
                        if (vdRes.rows.length > 0) {
                            ...
                            await client.query(`UPDATE venta_detalle 
                 SET venta_detalle_cantidad = $1, 
                     venta_detalle_subtotal = $2 
                 WHERE venta_detalle_id = $3`, [newQty, newSubtotal, vd.venta_detalle_id]);
                        }
                    }
                    // Recalcular total neto de la venta
                    const sumRes = await client.query(`SELECT COALESCE(SUM(venta_detalle_subtotal), 0) as total FROM venta_detalle WHERE venta_id = $1`, [ventaId]);
                    const newVentaTotal = parseFloat(sumRes.rows[0].total);
                    await client.query(`UPDATE venta 
             SET venta_total = $1, 
                 venta_observacion = 'Venta modificada por devolución parcial aprobada' 
             WHERE venta_id = $2`, [newVentaTotal, ventaId]);
                }
            }
         ...
         WHERE devolucion_id = $1`, [devolucionId]);
            ...
            if (error instanceof error_middleware_1.AppError)
                throw error;
            throw new error_middleware_1.AppError('Error al procesar la cancelación de la entrega', 500);
        }
        finally {
            client.release();
        }
    ...
    marcarNoEntregado: async (req, res) => {
        const client = await db_1.default.connect();
        try {
            const id = parseInt(req.params.id);
            ...
            const detailsRes = await client.query('SELECT * FROM solicitud_entrega_detalle WHERE solicitud_entrega_id = $1', [id]);
            const stockUpdates = [];
            for (const d of detailsRes.rows) {
                await client.query('UPDATE producto SET producto_stock = producto_stock + $1 WHERE producto_id = $2', [d.solicitud_entrega_detalle_cantidad, d.producto_id]);
                const prodRes = await client.query('SELECT producto_stock FROM producto WHERE producto_id = $1', [d.producto_id]);
                stockUpdates.push({
                    producto_id: d.producto_id,
         ...
         WHERE solicitud_entrega_id = $3`, [observaciones || 'No retirado por el empleado', foto_evidencia || null, id]);
         ...
         WHERE empleado_id = $1 
           AND sucursal_id = $2 
           AND ABS(EXTRACT(EPOCH FROM (venta_fecha - $3))) < 5
           AND venta_estado = 'completada'`, [solicitud.empleado_id, solicitud.sucursal_id, solicitud.solicitud_entrega_fecha_solicitud]);
            // Si existía una solicitud de devolución/cancelación pendiente para esta entrega, se rechaza automáticamente.
            await client.query(`UPDATE devolucion 
         SET devolucion_estado = 'rechazado', 
         ...
         WHERE solicitud_entrega_id = $1 AND devolucion_estado = 'pendiente'`, [id]);
            await client.query('COMMIT');
            if (req.io) {
                req.io.emit('entrega-realizada', { id });
                req.io.emit('devolucion-actualizada', { id });
            ...
            if (error instanceof error_middleware_1.AppError)
                throw error;
            throw new error_middleware_1.AppError('Error al registrar no entregado', 500);
        }
        finally {
            client.release();
        }
    },
    reportarIncidente: async (_req, res) => {
        res.json({ success: true, message: 'Incidente registrado' });
    },
    getEstadisticas: async (_req, res) => {
        const statsRes = await db_1.default.query(`
      SELECT 
